...
Page({
   ...
   */
  toggleMahjong() {
    const showMahjong = !this.data.showMahjong
    this.setData({
      showMahjong,
      showGame24: false,
      mahjongResult: null,
   ...
   */
  addMahjongTile(e) {
    if (this.data.mahjongHand.length >= 13) return
    
    const tile = e.currentTarget.dataset.tile
    
    // 统计当前手牌中该牌的数量
    const tileCount = this.data.mahjongHand.filter(t => t === tile).length
    if (tileCount >= 4) return // 每张牌最多只能出现4次
    
    const newHand = [...this.data.mahjongHand, tile]
    // 对手牌进行排序，便于查看
  ...
  removeMahjongTile(e) {
    ...
    const newHand = [...this.data.mahjongHand]
  ...
  checkMahjongHu() {
    ...
    console.log('开始判定胡牌，手牌:', hand)
    const result = this.calculateMahjongHu(hand)
    console.log('胡牌判定结果:', result)
    
    // 确保result结构正确
    const huCards = result.huCards || []
    ...
    // 格式化显示文本
    const huCardsText = huCards.length > 0 ? huCards.map(card => numToChinese[card] + '万').join('、') : '无'
    const huTypeText = huType.length > 0 ? huType.join('、') : '无'
    
    const finalResult = {
      isTing: result.isTing || false,
      huCards: huCards,
      huType: huType,
      huCardsText: huCardsText,
      huTypeText: huTypeText
    }
    
    console.log('最终设置的结果:', finalResult)
    this.setData({
      mahjongResult: finalResult
    })
  },
   ...
   * 计算可胡的牌
   */
  calculateMahjongHu(hand) {
    try {
      const huCards = []
      const huTypes = []
      
      // 依次尝试加入1-9万各1张，检查是否能胡牌
      for (let i = 1; i <= 9; i++) {
        // 检查加入该牌后是否超过4张
        const tileCount = hand.filter(t => t === i).length
        if (tileCount >= 4) {
          console.log('跳过测试胡牌:', i, '万，因为手牌中该牌已达4张')
          continue
        }
        
        const testHand = [...hand, i]
        testHand.sort((a, b) => a - b)
        console.log('测试胡牌:', i, '万，测试手牌:', testHand)
        
        const isRegularHu = this.checkRegularHu(testHand)
        console.log('常规胡牌判定:', isRegularHu)
        if (isRegularHu) {
          if (!huCards.includes(i)) {
            huCards.push(i)
            if (!huTypes.includes('平胡')) {
              huTypes.push('平胡')
            }
          }
        }
        
        const isSevenPairsHu = this.checkSevenPairsHu(testHand)
        console.log('七对胡牌判定:', isSevenPairsHu)
        if (isSevenPairsHu) {
          if (!huCards.includes(i)) {
            huCards.push(i)
            if (!huTypes.includes('七对')) {
              huTypes.push('七对')
            }
          }
        }
      ...
      console.log('最终可胡牌:', huCards, '胡牌类型:', huTypes)
      return {
        isTing: huCards.length > 0,
        huCards: huCards,
        huType: huTypes
      }
    } catch (error) {
      console.error('计算可胡牌时出错:', error)
      return {
        isTing: false,
        huCards: [],
        huType: []
  ...
  checkRegularHu(hand) {
    ...
    for (let i = 1; i <= 9; i++) {
      if (count[i] >= 2) {
        ...
        // 检查是否能组成4组顺子/刻子
        if (this.checkGroups(newCount)) {
          console.log('常规胡牌判定成功，将牌:', i, '万')
          return true
        }
      }
    }
  ...
  checkGroups(count) {
    ...
    // 处理顺子
    for (let i = 1; i <= 7; i++) {
      while (countCopy[i] > 0 && countCopy[i + 1] > 0 && countCopy[i + 2] > 0) {
        countCopy[i]--
        countCopy[i + 1]--
        countCopy[i + 2]--
        groups++
